...
if (!function_exists('admin_layout_start')) {
    function admin_layout_start($title, $activeMenu, $pageHeading, $pageSubheading = '', $headerActions = '', $extraHead = '')
    {
        ...
        echo '<style>';
        echo 'body{font-family:\'Prompt\',sans-serif;background:#f4f6fb;color:#1f2937;}';
        echo '.admin-shell{display:flex;min-height:100vh;}';
        echo '.admin-sidebar{width:260px;background:#fff;border-right:1px solid #e5e7eb;padding:22px 18px;position:sticky;top:0;height:100vh;display:flex;flex-direction:column;}';
        echo '.admin-brand{font-size:1.05rem;font-weight:600;color:#0d6efd;text-decoration:none;display:flex;align-items:center;gap:10px;margin-bottom:24px;}';
        echo '.admin-nav{display:flex;flex-direction:column;gap:6px;}';
        echo '.admin-nav-link{display:flex;align-items:center;gap:10px;border-radius:10px;padding:10px 12px;text-decoration:none;color:#4b5563;font-weight:500;}';
        echo '.admin-nav-link:hover{background:#eef4ff;color:#0d6efd;}';
        echo '.admin-nav-link.active{background:#0d6efd;color:#fff;box-shadow:0 8px 20px rgba(13,110,253,.2);}';
        echo '.admin-sidebar-footer{margin-top:auto;}';
        echo '.admin-main{flex:1;padding:20px 24px 28px;}';
        echo '.admin-topbar{background:#fff;border:1px solid #e5e7eb;border-radius:14px;padding:14px 18px;display:flex;justify-content:space-between;gap:18px;align-items:center;margin-bottom:18px;}';
        echo '.admin-page-title{margin:0;font-size:1.3rem;font-weight:600;}';
        echo '.admin-page-subtitle{margin:4px 0 0;color:#6b7280;font-size:.95rem;}';
        echo '.admin-content{display:flex;flex-direction:column;gap:16px;}';
        echo '.admin-surface{background:#fff;border:1px solid #e5e7eb;border-radius:14px;box-shadow:0 4px 14px rgba(17,24,39,.04);}';
        echo '@media (max-width: 991px){.admin-shell{flex-direction:column;}.admin-sidebar{position:relative;height:auto;width:100%;}.admin-main{padding:16px;}.admin-topbar{flex-direction:column;align-items:flex-start;}}';
        echo '</style>';
        echo $extraHead;
        echo '</head>';
        echo '<body>';
        ...
        echo '<a class="admin-brand" href="' . admin_escape(admin_url('admin_dashboard.php')) . '"><i class="fa-solid fa-store"></i><span>Moopik POS Admin</span></a>';
